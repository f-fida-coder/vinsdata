// VINVAULT shared UI primitives
import React, { useState, useEffect, useRef } from 'react';

export const Icon = ({ name, size = 16, stroke = 1.6, className = '', style }) => {
  const paths = {
...
export const TierDot = ({ tier }) => {
  // Accept either "T1" / "tier_1" / "Tier 1"
  const key = (() => {
  ...
  if (!key) return <span className="cell-muted">—</span>;
  const colors = { T1: 'var(--tier1)', T2: 'var(--tier2)', T3: 'var(--tier3)' };
  return (
    <span className="tier-pill">
      <span className="tier-dot" style={{ background: colors[key] }}/>
      {key}
    </span>
  );
};
...
const STATUS_VARIANT = {
  ...
  'Not interested': 'muted', 'Wrong number': 'muted', 'No answer': 'muted',
  'Voicemail left': 'neutral', 'Deal closed': 'success', Nurture: 'info',
  Disqualified: 'muted', 'Do not call': 'danger', Marketing: 'info',
  // file pipeline
  Complete: 'success', Processing: 'info', Filtering: 'warn', Carfax: 'neutral',
  Generated: 'neutral', Active: 'success', Scheduled: 'info', Completed: 'muted',
...
export const StatusBadge = ({ status, label }) => {
  ...
  const display = label || (typeof status === 'string' ? status.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase()) : status);
  return <span className={`status-badge sb-${variant}`}>{display}</span>;
};

export const TempPill = ({ temp }) => {
  if (!temp || temp === '—') return <span className="temp-pill empty">— Not set —</span>;
  const map = {
    Hot:  { c: 'var(--hot)' }, hot:  { c: 'var(--hot)' },
    Warm: { c: 'var(--warm)' }, warm: { c: 'var(--warm)' },
    Cold: { c: 'var(--cold)' }, cold: { c: 'var(--cold)' },
    'No answer': { c: 'var(--muted-fg)' }, no_answer: { c: 'var(--muted-fg)' },
    Closed: { c: 'var(--text-2)' }, closed: { c: 'var(--text-2)' },
  };
  const t = map[temp] || map.Cold;
  const display = String(temp).replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
  return (
...
export const PriorityChip = ({ priority }) => {
  ...
  const display = k.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
  return <span className={`priority-chip pc-${k}`}>{display}</span>;
};

export const Avatar = ({ name, size = 24, color, style: extraStyle }) => {
  ...
  return (
    <span className="avatar" style={{ width: size, height: size, background: bg, fontSize: Math.round(size * 0.42), ...(extraStyle || {}) }}>
      {initial}
    </span>
  );
};

export const Button = ({ variant = 'secondary', size = 'md', children, icon, iconAfter, onClick, type, disabled, className = '', ...rest }) => (
  <button
    type={type || 'button'}
    onClick={onClick}
    disabled={disabled}
    className={`vv-btn vv-btn-${variant} vv-btn-${size} ${className}`}
    {...rest}
  >
...
export const Input = ({ icon, className = '', ...rest }) => (
  <div className="vv-input-wrap">
    {icon && <Icon name={icon} size={15} className="vv-input-icon"/>}
    <input className={`vv-input ${icon ? 'has-icon' : ''} ${className}`} {...rest}/>
  </div>
);

export const StageDots = ({ stages }) => (
...
export const KPI = ({ label, value, hint, dot, trend, trendDir, large, onClick }) => (
  <div className={`kpi ${large ? 'kpi-lg' : ''}`} onClick={onClick}>
...
export const Bar = ({ label, count, max, color = 'var(--text-0)', onClick }) => (
  <div className="bar-row" onClick={onClick}>
