...
if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
    ...
    if ($id <= 0) {
        pipelineFail(400, 'id is required', 'missing_id');
    }

    $stmt = $db->prepare('SELECT id, batch_name FROM lead_import_batches WHERE id = :id');
    $stmt->execute([':id' => $id]);
    $batch = $stmt->fetch();
    if (!$batch) {
        pipelineFail(404, 'Batch not found', 'batch_not_found');
    }

    // imported_leads_raw cascades from this delete; lead_states / lead_labels /
    // lead_notes / lead_activities / lead_tasks / lead_contact_logs /
    // marketing_campaign_recipients / notifications / merge_prep_choices /
    // lead_duplicate_group_members all cascade from imported_leads_raw.
    // merge_prep_groups.preferred_primary_lead_id is ON DELETE SET NULL.
    $stmt = $db->prepare(
        'SELECT COUNT(*) FROM imported_leads_raw WHERE batch_id = :bid'
    );
    $stmt->execute([':bid' => $id]);
    $leadCount = (int) $stmt->fetchColumn();

    $del = $db->prepare('DELETE FROM lead_import_batches WHERE id = :id');
    $del->execute([':id' => $id]);
