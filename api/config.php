...
if (PHP_SAPI !== 'cli') {
    // Never let PHP warnings/notices leak into the JSON body. PHP 8.5 (local
    // dev) deprecated PDO::MYSQL_ATTR_INIT_COMMAND with a chatty HTML notice
    // that prefixed every response and broke axios JSON parsing on login.
    // Production already had display_errors off; this just makes dev match.
